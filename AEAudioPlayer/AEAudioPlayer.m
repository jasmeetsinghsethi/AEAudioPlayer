...
{
    if( _delegate != nil && [_delegate respondsToSelector:@selector(audioPlayerRouteChange:reason:previousRoute:)])
    {
        [_delegate audioPlayerRouteChange:self
                                   reason:[notification.userInfo[AVAudioSessionRouteChangeReasonKey] intValue]
                            previousRoute:notification.userInfo[AVAudioSessionRouteChangePreviousRouteKey]];
    }
}
...
- (void)startBufferingURL:(NSURL*)url AtOffset:(AVAudioFramePosition)offset error:(NSError**)outError
{
    ...
    if(!engine.isRunning)
    {
        [self startEngineError:outError];
    }
...
- (void)startEngineError:(NSError**)error
{
    NSLog(@"STARTING ENGINES");
    [[self.class sharedEngine] startAndReturnError:error];
    if(error && *error)
    {
